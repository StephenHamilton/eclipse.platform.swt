...
long /*int*/ on_before_popup(long /*int*/ browser, long /*int*/ frame, final long /*int*/ target_url, long /*int*/ target_frame_name, long /*int*/ pPopupFeatures, long /*int*/ windowInfo, long /*int*/ client, long /*int*/ settings, long /*int*/ no_javascript_access) {
	...
	CEF3.memmove(windowInfo, pPopupFeatures, cef_popup_features_t.sizeof);
	final Point location;
	if (popupFeatures.ySet == 1 && popupFeatures.xSet == 1) {
		location = new Point(popupFeatures.x, popupFeatures.y);
	}
	else {
		location = null;
	}
	...
	if (popupFeatures.widthSet == 1) {
		if (popupFeatures.heightSet == 1) {
			size = new Point(popupFeatures.width, popupFeatures.height);
		}
		else {
			size = new Point(popupFeatures.width, host.browser.getSize().y);
		}
	}
	else if (popupFeatures.heightSet == 1) {
		size = new Point(host.browser.getSize().y, popupFeatures.height);
	}
	else {
		size = null;
	}
	final boolean addressBar = popupFeatures.locationBarVisible == 1;
	final boolean menuBar = popupFeatures.menuBarVisible == 1;
	final boolean statusBar = popupFeatures.statusBarVisible == 1;
	final boolean toolBar = popupFeatures.toolBarVisible == 1;
	final int result[] = new int[1];
	result[0] = 0;
	host.browser.getDisplay().syncExec(new Runnable() {
		public void run() {
			if (host.browser.isDisposed()) return;
			result[0] = host.onWindowOpen(target_url, addressBar, menuBar, statusBar, toolBar, location, size);
		}
	});
